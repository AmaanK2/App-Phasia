...
# Resets the Crossword Puzzle Worksheet window and the points variable
def res():
    global words, answers, num, points
    points = 0
    e1.delete(0, END)
    my_image = Image.open('Crossword Puzzle Worksheet.jpg')
...
# The down and across functions prints out a specific clue from the words list, and that clue corresponds to the num integer.
def down1():
    global words, answers, num
    num = 0
    lbl.config(text=words[num])
...
# This replaces the original tkinter icon with an icon of your choice
window = Tk()
window.geometry("750x850+400+300")
window.iconbitmap('Crossword Puzzle Icon.ico')
window.title("Crossword Puzzle")

# This generates the crossword puzzle window
my_image = Image.open('Crossword Puzzle Worksheet.jpg')
my_imager = ImageTk.PhotoImage(my_image)
my_image_label = Label(image=my_imager)
my_image_label.image = my_imager
...
root.title("Pictures/AphasiaTherapy")
root.configure(background="orange")
root.iconbitmap('Crossword Puzzle Icon.ico')
